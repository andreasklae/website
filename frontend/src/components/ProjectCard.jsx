        ...
        <div className="flex flex-wrap gap-2 mb-6 mx-6">
          {project.type === 'website' ? (
            <>
              ...
              >
                <span>{getText(project.linkLabel || { en: 'Visit Website', no: 'Besøk nettside' })}</span>
        ...
        {!photosLoading && projectPhotos && projectPhotos.length > 0 && (
          ...
          >
            <div className="border-t border-ide-border pt-6">
              ...
              <div className="mb-6 mx-6">
                ...
                <div className="text-gray-300 text-base leading-relaxed text-wrap break-words">
                  ...
                  {project.id === '3dpathfinding' && (
                    <div>
                      {getText({
                        en: "We benchmark A*, Dijkstra, Breadth‑First Search and Theta* on procedurally generated 3D voxel mazes. The project combines maze generation and algorithm implementations with a focused analysis pipeline for clear, reproducible comparisons.",
                        no: "Vi benchmarker A*, Dijkstra, Breadth‑First Search og Theta* på prosedyregenererte 3D‑voxel‑labyrinter. Prosjektet kombinerer labyrintgenerering og algoritmeimplementasjoner med en fokusert analyse‑pipeline for klare, reproduserbare sammenligninger."
                      })}
                      <br /><br />
                      {getText({
                        en: "Seeded batch runs sweep across distance categories and obstacle densities. For each run we record execution time, explored nodes, path length and path efficiency, export results to CSV, and visualize them in notebooks and an interactive Plotly 3D model.",
                        no: "Seedede batch‑kjøringer går på tvers av avstandskategorier og hindringstetthet. For hver kjøring registrerer vi kjøretid, utforskede noder, rutelengde og ruteeffektivitet, eksporterer resultatene til CSV og visualiserer dem i notatbøker og en interaktiv 3D‑modell i Plotly."
                      })}
                      <br /><br />
                      {getText({
                        en: "Built with Python, NumPy, Plotly and pathfinding3d, organized for repeatable experiments and easy extension with new heuristics or movement models.",
                        no: "Bygget med Python, NumPy, Plotly og pathfinding3d, og organisert for repeterbare eksperimenter og enkel utvidelse med nye heuristikker eller bevegelsesmodeller."
                      })}
                    </div>
                  )}
                  {project.id === 'in1060' && (
              ...
              {/* Credits Section */}
              {(project.id === 'in1060' || project.id === 'in2000' || project.id === 'ml-project' || project.id === '3dpathfinding') && (
                <div className="mb-6 mx-6">
                  <h4 className="text-lg font-semibold text-white mb-3">
                  ...
                  <div className="text-gray-300 text-sm">
                    ...
                    {project.id === '3dpathfinding' && (
                      <ul className="space-y-1">
                        <li>• Andreas Klæboe – {getText({ en: '(me)', no: '(meg)' })}</li>
                        <li>• Belal Zaidan – {getText({ en: 'Team member', no: 'Gruppemedlem' })}</li>
                        <li>• Leon Hennie‑Johansen – {getText({ en: 'Team member', no: 'Gruppemedlem' })}</li>
                      </ul>
                    )}
                  </div>
                </div>
